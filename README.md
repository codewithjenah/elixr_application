...
| Explicit device           | `camera_device_id` from discovery                     | Tries that device first. If it cannot resolve, open, or produce stable usable frames during preparation, temporarily tries the normal Auto-select path and reports the actual camera in the prepare acknowledgment. |
...
Camera preferences are stored in local settings (`%APPDATA%\Elixr\settings.json` on Windows) as `camera_device_id` and a cached `camera_display_name`. A preparation-time fallback is session-local: Flutter warns the trainee, does not overwrite either saved field, and retries the original selected device first on the next new session. If neither the selected device nor any Auto-select candidate is usable, practice keeps the fatal `selected_camera_unavailable` recovery flow (`camera_unavailable` remains the Auto-select failure code).
...
2. **Camera/session preparation** — Flutter sends a protocol v1 `prepare` command with `request_id`, `session_id`, movement, difficulty, selected `prop_type`, `bottle_detection_enabled`, and camera selection (`camera_device_id` or legacy `camera_index`). The backend opens the requested camera first. An unavailable explicit `camera_device_id` may temporarily fall back through Auto-select before the backend returns a correlated `command_ack` with `accepted: true`, `session_state: "preparing"`, `selected_camera_fallback_used`, and the actual camera identity/name when available. Flutter shows one non-blocking warning for that accepted preparation. Preview frames stream without scoring or detectors.
...
- If a saved camera is unplugged or busy, preparation tries Auto-select temporarily and identifies the active camera in a trainee warning. The saved selection remains unchanged and is retried next session. If no candidate works, the backend returns `selected_camera_unavailable`.
