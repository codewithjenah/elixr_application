...
- `backend/models/best.onnx` — FP32 ONNX export of the same weights for
  ONNX Runtime CPU inference. Generated by `backend/scripts/export_prop_onnx.py`.
  `YOLO_RUNTIME=auto` keeps PyTorch after CPU benchmarking; set
  `YOLO_RUNTIME=onnx_cpu` to opt into ONNX Runtime.
