...
4. **Readiness check** — After the first JPEG, guided practice sends protocol v1 `begin_readiness`. The backend loads detectors on the same camera session (Pose is loaded for every guided movement so proximity calibration can sample shoulder width, without changing checklist items), streams annotated frames with `session_state: "readying"`, and emits optional checklist fields (`readiness_items`, `readiness_complete`, `readiness_stable`, `readiness_stable_progress`) plus `calibration_scale` / `calibration_source` once a measurement exists. Readiness validates **camera, prop, and landmark observability only** — it is not technique coaching and does not score, update hold confirmation, or evaluate movement success thresholds.
