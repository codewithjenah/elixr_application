...
- Local computer vision for fifteen movements:
  - Easy: Normal Grip, Bartender's Grip, Reverse Grip, Claw Grip, Body Grip
    ...
    Body Grip wraps one hand around the bottle body/middle)
  - Medium: Hand Stall, One Finger Stall, Forearm Stall, Elbow Stall, Wrist Stall
    (One Finger Stall balances one upright selected prop on the thenar eminence, with the index finger extended horizontally;
    Wrist Stall balances the selected prop around either wrist)
  - Hard: Reverse Forearm Stall, Shoulder Stall, Double Hand Stall, Bottle in a tin, Double Forearm Stall
    ...
    Double Forearm Stall balances one bottle on each forearm at the same time)
...
python -c "import cv2, mediapipe, numpy, ultralytics, onnxruntime; print('backend imports OK')"
cd ..
```
...
- `backend/models/best.onnx` — static FP32 ONNX export of the same weights
  for ONNX Runtime CPU inference. Production contract is
  `[1, 3, 480, 640]` (ELIXR camera frames are 640×480). Generated by
  `backend/scripts/export_prop_onnx.py` with `imgsz=(480, 640)`,
  `dynamic=False`, opset 17, no NMS. `YOLO_RUNTIME=auto` prefers
  validated ONNX CPU when `best.onnx` and ONNX Runtime are available,
  and falls back to PyTorch. DirectML remains explicit opt-in via
  `YOLO_RUNTIME=onnx_dml`. Do not export a square 640×640 graph for production.
...
| Explicit device           | Stable `camera_device_id` from discovery              | Tries only that physical device, including bounded same-device reopen/profile recovery. If it cannot resolve, open, or produce stable usable frames, preparation fails with `selected_camera_unavailable`. Entries with `identity_stable: false` (including `opencv:N`) are Auto-select-only. |
...
Stable camera preferences are stored in local settings (`%APPDATA%\Elixr\settings.json` on Windows) as `camera_device_id` and a cached `camera_display_name`. Unstable identities such as OpenCV fallback IDs remain visible for discovery but cannot be persisted as an explicit physical selection; use Auto-select for those devices. A failed explicit preparation never overwrites either saved field and never opens a different physical camera; practice keeps the fatal `selected_camera_unavailable` recovery flow. `camera_unavailable` remains the Auto-select failure code.
...
2. **Camera/session preparation** — Flutter sends a protocol v1 `prepare` command with `request_id`, `session_id`, movement, difficulty, selected `prop_type`, `bottle_detection_enabled`, and camera selection (`camera_device_id` or legacy `camera_index`). A non-null stable `camera_device_id` is authoritative and can only reuse or reopen that physical device; if bounded same-device startup fails, the backend rejects preparation with `selected_camera_unavailable`. Ephemeral `opencv:N` IDs are not accepted as explicit physical selections. Only `camera_device_id: null` enables Auto-select fallback. Accepted preparation returns a correlated `command_ack` with `session_state: "preparing"` and the active camera identity/name when available. Preview frames stream without scoring or detectors.
...
4. **Readiness check** — After the first JPEG is sent, the backend asynchronously warms the prop model and only the landmark detectors required for that movement (`readiness_needs_hands` / `readiness_needs_pose`; Free Practice loads neither) on the same camera session. Guided practice sends protocol v1 `begin_readiness`, which reuses or waits for that single warm-up. Warm-up observations are discarded; readiness tracking begins from fresh readying frames. Calibration samples the already-running modality (palm length or shoulder width) and does not start an extra detector solely for scale. The session streams annotated frames with `session_state: "readying"`, and emits optional checklist fields (`readiness_items`, `readiness_complete`, `readiness_stable`, `readiness_stable_progress`) plus `calibration_scale` / `calibration_source` once a measurement exists. Readiness validates **camera, prop, and landmark observability only** — it is not technique coaching and does not score, update hold confirmation, or evaluate movement success thresholds.
5. **Auto-start after Ready beat** — After `readiness_stable` is true (consecutive per-item frames plus a monotonic stable duration), Flutter waits a short Ready beat (~500 ms), then automatically sends protocol v1 `confirm_readiness`. The backend accepts only when the latest readiness snapshot is currently stable, then Flutter freezes the checklist and enters `PracticeRunPhase.countdown`. Ordinary detection loss during countdown does **not** cancel countdown or revoke confirmation; only fatal camera/backend/model errors abort. Losing stability during the Ready beat cancels auto-start and re-arms when stable again.
...
- If a saved camera is unplugged or busy, preparation stays on that selected physical device and returns `selected_camera_unavailable` after bounded same-device recovery. The saved selection remains unchanged; the user can retry, choose another discovered camera, or intentionally select Auto-select.
