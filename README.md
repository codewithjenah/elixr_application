...
- `backend/models/best.onnx` — static FP32 ONNX export of the same weights
  ...
  `dynamic=False`, opset 17, no NMS. `YOLO_RUNTIME=auto` keeps PyTorch
  after CPU benchmarking; set `YOLO_RUNTIME=onnx_cpu` to opt into ONNX
  Runtime. Do not export a square 640×640 graph for production.
