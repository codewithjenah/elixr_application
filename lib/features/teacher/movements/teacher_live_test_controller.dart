...
/// Ephemeral Teacher Live Test session. Owns WebSocket lifecycle only.
///
/// Does not write sessions, assignment attempts, XP, leaderboard, or video.
class TeacherLiveTestController extends ChangeNotifier {
  ...
  bool get testSucceeded => _testSucceeded;
  ...
  Future<void> startSession() async {
    ...
    try {
      final ack = await _ws.sendPrepare(
        ...
        assessmentSpec: draft.assessmentSpec,
      );
      if (_disposed || !_run.isPreparingCamera) return;
      if (!ack.accepted) {
        _failPrepare(
  ...
  Future<void> confirmReadiness() async {
    if (_disposed || _commandInFlight || !canStartTest) {
      return;
    }
    ...
    try {
      ...
      if (!ack.accepted) {
        if (ack.errorCode == 'readiness_not_confirmed') {
          ...
          _errorMessage =
              ack.message ??
              'Readiness must be confirmed before the test can start.';
          _notify();
          return;
        }
        _failPrepare(
  ...
  String? _friendlyErrorCode(String? code) {
    return switch (code) {
      ...
      'ack_action_mismatch' || 'ack_session_mismatch' =>
        'The backend response did not match this test. Try starting again.',
      _ => null,
    };
  }
