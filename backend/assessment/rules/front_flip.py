...
def evaluate(
    ...
    if bottle is None:
        return (
            RuleResult(
                feedback="Keep the bottle visible for the front flip.",
                feedback_type="error",
    ...
    flip_detected = detect_vertical_flip(state)
    ...
    if phase == "caught":
        result = RuleResult(
            feedback="Front flip caught cleanly.",
            feedback_type="positive",
            posture_status="stable",
        )
    elif phase == "air":
        result = RuleResult(
            feedback="Bottle in the air. Prepare to catch.",
            feedback_type="warning",
            posture_status="stable",
        )
    elif near_hand:
        result = RuleResult(
            feedback="Release with a forward flip motion.",
            feedback_type="warning",
            posture_status="stable",
        )
    else:
        result = RuleResult(
            feedback="Flip the bottle forward and catch.",
            feedback_type="warning",
