...
from config import (
    DOUBLE_FOREARM_MAX_SCALED_CONTACT_DISTANCE,
...
def _pair_bottles_to_forearms(
    ...
    c0 = b0.bottom_center_normalized(640, 480)
    c1 = b1.bottom_center_normalized(640, 480)
...
def _bottle_on_forearm(
    ...
    *,
    ...
    return geometry is not None and (
        ...
        <= min(
            scaled_proximity(
                FOREARM_STALL_MAX_CONTACT_DISTANCE, movement_state
            ),
            DOUBLE_FOREARM_MAX_SCALED_CONTACT_DISTANCE,
        )
    )
...
def evaluate(
    ...
    *,
    ...
    left_elbow = pose.get(13)
    left_wrist = pose.get(15)
    right_elbow = pose.get(14)
    right_wrist = pose.get(16)
    if (
        left_elbow is None
    ...
    left_contact = left_bottle.bottom_center_normalized(640, 480)
    right_contact = right_bottle.bottom_center_normalized(640, 480)
    left_to_left = _dist(left_contact, left_mid)
    left_to_right = _dist(left_contact, right_mid)
    right_to_left = _dist(right_contact, left_mid)
    right_to_right = _dist(right_contact, right_mid)
    ...
    left_valid = _bottle_on_forearm(
        left_bottle, left_elbow, left_wrist, movement_state=current
    )
    right_valid = _bottle_on_forearm(
        right_bottle, right_elbow, right_wrist, movement_state=current
    )
